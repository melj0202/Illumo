...
process exit code. IllumoGame, IllEd, and IllMeshViewer supply declarative product metadata and
...
int RunIllumoApplication(int argc, char** argv,
  ...
  illumo.addModule(make_unique<DebugModule>(), Optional); // Debug / RelWithDebInfo
...
\end{lstlisting}
The engine-owned platform entry obtains the product definition from
...
configuration-file path. Test-only access injects the host's private window and backend factories.
...
starts the incoming module. The engine runner registers the product-supplied initial
module as required and engine-owned Debug/RelWithDebInfo \symbolref{DebugModule} as optional.
