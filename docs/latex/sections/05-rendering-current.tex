...
IllEd's document is the mutation gateway to its authoritative runtime graph;
serialization recipes and exact local-box picking remain editor policy.
Transforms and recolors preserve graph handles and visual objects. Journal
overflow resynchronizes bindings without rebuilding nodes. \texttt{.ilsc} v1
is unchanged. The simulator diagnostic and capture fixtures use the adapter;
CA storage and UI remain separate. See
\pathref{docs/scene-graph-v2-design.md} and
\pathref{docs/scene-graph-v2-plan.md} for evidence and gated follow-ups.
