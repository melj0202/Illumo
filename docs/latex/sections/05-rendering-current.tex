...
\begin{itemize}
  ...
  \item \symbolref{MeshVisual} (D-R21) is the world mesh host and
        ...
        directional lighting with a depth-only shadow pass, and optional
        object motion blur from the previous clip pose. It clones
        ...
        one attachment. Overlay HUD stays on \symbolref{GameVisual}. Triangle
        geometry retains unique vertices and its source index topology in
        independently sized dynamic VBO and EBO storage. Dirty geometry emits
        vertex and index update tokens; unchanged frames reuse both buffers,
        while capacity growth replaces the backing allocation without changing
        its mesh handle.
\end{itemize}
