...
\begin{itemize}
  ...
  \item \symbolref{DebugDraw3D} is a deliberately narrow token drawable for
        ...
        perspective view-projection matrix and optional model matrix; it owns
        no camera, materials,
        model loading, lighting, or scene system. Compose it in World before
        2D presentation, which remains painter ordered in one main pass.
\end{itemize}
...
\begin{itemize}
  ...
  \item GPU: one reusable nearest-filtered RGB display texture and one
        ...
        destruction explicitly releases the texture.
...
\texttt{IllumoTests} owns reusable rendering/services cases, while
