...
\begin{itemize}
  ...
  \item \symbolref{DebugDraw3D} is a deliberately narrow token drawable and
        \symbolref{ISceneRenderAttachment} for
        colored axes, ground grids, wire cubes, and solid cubes. It reuses the
        existing position-plus-RGBA mesh layout and Shape shader through local
        depth-tested line and triangle styles. Callers supply a complete
        perspective view-projection matrix and optional model matrix. When it
        is attached to a scene node, it composes node-world and local model
        matrices without mutating either; it owns
        no camera, materials,
        model loading, or lighting. Compose it in World before
        2D presentation, which remains painter ordered in one main pass. Its
        line and triangle meshes are dynamic retained resources: geometry changes
        upload vertices through \texttt{UpdateBuffer}, while capacity growth alone
        replaces the backing allocation.
\end{itemize}
