...
\begin{enumerate}
  ...
  \item \symbolref{Renderer::RenderScene}:
        collect visible World shadow casters; when present, fit and render one
        shared depth pass; then clear + viewport tokens and walk layers
...
\begin{itemize}
  ...
  \item \symbolref{MeshVisual} (D-R21) is the world mesh host and
        ...
        Directional shadow ownership is scene-level (D-R22): visible direct
        meshes and SceneGraph attachments contribute transformed caster bounds
        and depth tokens; Renderer reuses one framebuffer, fits one shared
        light-space matrix to the combined bounds, clears once, and exposes the
        same depth texture to every receiver. MeshVisual owns no shadow FBO.
        Compose complex objects as child scene nodes, each with
