...
comparison. Retain up to 4,096 changed chunk addresses, expand them by one
...
it required all 256 halo cells.\\
...
mixed stepping. The 4,096-address retention cap bounds persistent bookkeeping;
crossing it invalidates the frontier for that generation. Focused coverage
exercises more than 64 frontier targets, candidate selection, forced-complete
byte identity, local-source bounds, and broad-change fallback.\\
...
visible changed chunk, while fade work follows colors still moving. Revision
gaps, density overviews, camera or palette changes, whole-grid replacement,
and invalid change journals deliberately use the complete bounded resample.
...
overview and revision-gap fallback, active fade counts, and repeated zero-speed
configuration.\\
...
\textbf{Consequences:} Sub-cache camera motion changes only the MVP. Cache exit,
LOD/resize, palette changes, revision gaps, and replacement trigger exact
bounded refills. Status exposes visible/cache dimensions, LOD, refill count,
and rolling refill latency; deterministic and warmed camera tests cover reuse,
boundaries, hysteresis, and overview updates.\\
...
changes plus a greater-than-4,096-change full replacement. In two comparable
...
marker without chunk snapshots. Let the spare grid read the immutable published
