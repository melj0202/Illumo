// IllumoGame headless test runner. CTest invokes one case per process.
               ...
               const std::string& workingDirectory)
{
  ...
  output << "# Generated by IllumoGameTests --write-ctest.\n";
  for (const IllumoTestCase& testCase : registry.getTestCases()) {
    ...
    const std::filesystem::path caseDirectory =
      std::filesystem::path(workingDirectory) / "Testing" / "IllumoGame" /
      safeName;
    ...
    output << "set_tests_properties([==[" << testCase.name
           ...
           << "]==] LABELS [==[IllumoGame;IllumoWorkspace]==] TIMEOUT "
...
main(int argc, char** argv)
{
  ...
  if (command == "--run" && argc == 3) {
    ...
    if (testCase == nullptr) {
      std::printf("Unknown IllumoGame test case: %s\n", argv[2]);
      return 2;
    }
  ...
  std::printf("Usage: IllumoGameTests [--list | --run <name> | --write-ctest "
